 ...
 * FactoryBean实现的简单模板超类，根据标志创建单例或原型对象。
 *
 ...
 * 如果“单例”标志为true （默认值），该类将创建它在初始化时只创建一次的对象，然后在对getObject()方法的所有调用中返回所述单例实例。
 *
 ...
 * 否则，每次调用getObject()方法时，此类都会创建一个新实例。 子类负责实现抽象的createInstance()模板方法以实际创建要公开的对象。
 *
